...
df1 = df1[df1["Adjective Response"]==adjective_resp]
df2 = df2[df2["Adjective Response"]==adjective_resp]

topic = topic_register[9]
df1 = df1[df1["Topics"] == topic]
df2 = df2[df2["Topics"] == topic]
...
for i, adj in enumerate(adjectives_init):
    # Filter the dataframes by adjective
    df1_adj = df1[df1['Adjective Initial'] == adj]
    df2_adj = df2[df2['Adjective Initial'] == adj]
    ...
    axs[i].set_title(f"Hist {adjective_resp} given {adj} \n- v1 {topic}", fontsize=8)
    axs[i+5].set_title(f"Hist {adjective_resp} given {adj}\n- v2 {topic}", fontsize=8)
...
plt.tight_layout()
plt.savefig(f"{topic}_interesting.png")
plt.show()
